...
function Keranjang({ cart }) {
    ...
    useEffect(() => {
        const newTotalPrice = CART.map(item => item.price * item.quantity).reduce((total, value) => total + value, 0);
        setTotalPrice(newTotalPrice);
    }, [CART]);
    console.log(CART)

    return (
        <div className="w-[517px] h-screen fixed right-0 top-0 pl-[22px] py-[65px] pr-[43px] overflow-y-auto">
                    ...
                    CART?.map((cartItem, cartIndex) => {
                        return (
                            <div className='w-[452px] h-[148px] relative border-b-[2px] border-zinc-200'>
                                <img src={cartItem.url} className='w-[106px] h-[106px] left-[29px] top-[21px] absolute'/>
                                <span className='left-[168px] top-[21px] absolute text-black text-2xl font-bold leading-[30px]'>{cartItem.name}</span>
                                ...
                                <span className='left-[168px] top-[55px] absolute text-black text-lg font-normal leading-[27px]'>Rp{cartItem.price * cartItem.quantity}</span>
                            </div>
                        )
                    })
                }
